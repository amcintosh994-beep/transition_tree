...
def lint_child(child: Dict[str, Any]) -> List[LintIssue]:
    ...
    if node_type not in NODE_TYPES:
        issues.append(LintIssue("error", "bad_type", f"Invalid node type: {node_type}"))

    affect = contains_affect(text)
    if affect:
        issues.append(LintIssue("error", "affect", f"Affect term detected ('{affect}'); track affect outside the tree."))

    hits = cue_hits(text)
    # Blend rule: if more than one type-cue is present, likely blended
    if len(set(hits)) > 1:
        issues.append(LintIssue("error", "blend", f"Multiple type cues detected {sorted(set(hits))}; split into separate nodes."))

    # Blocked nodes should name a blocker kind
    if status == "blocked" and blocker is None:
        issues.append(LintIssue("error", "blocked_needs_blocker", "status=blocked requires blocker_kind != null."))

    # Soft “atomicity” check
    if CONJUNCTION_RE.search(text):
        issues.append(LintIssue("warning", "conjunction", "Conjunction detected; verify this is a single atomic unit."))

    # Soft sanity: declared type should match cues if there is a cue
    if len(hits) == 1 and hits[0] != node_type:
        issues.append(LintIssue("warning", "type_mismatch",
                                f"Declared type={node_type} but cues suggest {hits[0]}; rewrite into canonical form if needed."))
...
def expand_node(db_path: str, node_id: str, model: str, max_repairs: int = 2) -> None:
    ...
    with conn:
        ...
        domain_name = conn.execute("SELECT name FROM domains WHERE id = ?", (parent["domain_id"],)).fetchone()["name"]

        proposal = openai_propose_children(db_path, parent, domain_name, model=model)

        # Deterministic lint + normalize (and build a repair plan)
        def lint_proposal(p: Dict[str, Any]) -> Tuple[Dict[int, List[str]], List[Dict[str, Any]]]:
            ...
            for i, child in enumerate(p["children"]):
                issues = lint_child(child)
                # Normalize text regardless; keep deterministic
                child2 = dict(child)
        ...
        while bad and repairs < max_repairs:
            proposal = openai_repair_children(bad, proposal, domain_name, model=model)
            bad, normalized = lint_proposal(proposal)
            proposal["children"] = normalized
            repairs += 1

        # If still bad after repairs, print the report and do not insert anything.
        if bad:
            print("Lint failed after repairs. Nothing inserted.")
            print(json.dumps(bad, indent=2))
            return

        # Insert children into SQLite
        created = now_iso()
