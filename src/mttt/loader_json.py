...
def load_nodes_edges_from_dir(dir_path: str | Path) -> Tuple[List[Node], List[Edge]]:
    ...
    for obj in nodes_raw:
        ...
        try:
            validate_domain(domain, field_name="facets.domain")
        ...
        facets = Facets(
            status=_parse_enum(
                Status,
                facets_obj.get("status", Status.ACTIVE.value),
                "facets.status",
            ),
            commitment=_parse_enum(
                Commitment,
                facets_obj.get("commitment"),
                "facets.commitment",
            ),
            recurring=bool(facets_obj.get("recurring", False)),
            frequency=facets_obj.get("frequency"),
            control_locus=_parse_enum(
                ControlLocus,
                facets_obj.get("control_locus"),
                "facets.control_locus",
            ),
            domain=domain,
